...
bool HistoryInner::hasSelectRestriction() const {
	// vlmGram fix: we make this function always retunring false, so we can
	// select every message, even if it is restricted or no, so it work
	// properly for channels where only AyuForward's is possible
	return false;
}
...
void HistoryInner::showContextMenu(QContextMenuEvent *e, bool showFromTouch) {
	...
	if (lnkPhoto || lnkDocument) {
		...
		const auto canForward = item && true;
...
bool CanSendReply(not_null<const HistoryItem*> item) {
	if (item->isDeleted() || (item->isEphemeral() && item->out())) {
		return false;
	}
	const auto peer = item->history()->peer;
