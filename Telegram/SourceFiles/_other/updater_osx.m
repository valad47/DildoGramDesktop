...
NSString *appName = @"DildoGram.app";
NSString *appDir = nil;
...
void RemoveQuarantineFromBundle(NSString *path) {
	...
	RemoveQuarantineAttribute([path stringByAppendingString:@"/Contents/MacOS/DildoGram"]);
...
int main(int argc, const char * argv[]) {
	...
	if (update) {
		...
		// Take the Updater (this currently running binary) from the place where it was placed by DildoGram
		// and copy it to the folder with the new version of the app (ready),
		// so it won't be deleted when we will clear the "DildoGram.app/Contents" folder.
