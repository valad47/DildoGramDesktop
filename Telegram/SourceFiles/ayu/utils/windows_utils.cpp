...
void processLegacy(const QString &appdata, const QString &iconPath) {
	auto shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/vlmGram Desktop.lnk";
	if (!QFile::exists(shortcut)) {
		shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/vlmGram.lnk";
	}
	if (!QFile::exists(shortcut)) {
		return;
	}
...
void processNewShortcuts(const QString &iconPath) {
	...
	const auto shortcut = path + u"vlmGram Desktop/vlmGram.lnk"_q;
	const auto native = QDir::toNativeSeparators(path).toStdWString();
...
void reloadAppIconFromTaskBar() {
	...
	QString iconPath = appdata + "/vlmGram.ico";
